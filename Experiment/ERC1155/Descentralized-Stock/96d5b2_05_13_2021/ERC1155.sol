...
contract ERC1155 is IERC1155, IERC1155TokenReceiver{
    ...
    /// @notice postcondition balances.length == _ids.length 
    /// @notice postcondition balances.length == _owners.length
    /// @notice postcondition forall (uint x) !( 0 <= x &&  x < balances.length ) || balances[x] == _balance[_owners[x]][_ids[x]]
    function balanceOfBatch(address[] memory _owners, uint256[] memory _ids) public  view returns (uint256[] memory balances){
