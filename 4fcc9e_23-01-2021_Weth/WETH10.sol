...
contract WETH10  {
    ...
    function flashLoan(IERC3156FlashBorrower receiver, address token, uint256 value, bytes calldata data) external  returns(bool) {
        ...
        require(value <= uint112(-1) && address(this).balance + _flashMinted + value <= uint112(-1), "WETH: supply limit exceeded");
    ...
    function _mintTo(address to, uint256 value) internal {
        require(address(this).balance + flashMinted <= uint112(-1), "WETH: supply limit exceeded");
        balanceOf[to] += value;
